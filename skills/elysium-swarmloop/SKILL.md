...
| 2+ compression triggers → context saturated, reduce subagents
...
4. Third timeout → grep for patterns, return PARTIAL with explicit gaps
5. HARD RULE: Never 0/100 — always produce SOMETHING (5/100 > 0/100). PARTIAL is always acceptable, never silent.
6. PRE-EMPTIVE SAVE: subagent writes .partial file every 120s → on timeout, read it for what was completed
```
...
- See `references/STATUS_CONTRACT.md` for the full status taxonomy.
...
## Phase 8 — Knowledge Persistence (gated — conditional loading)

Phase 8 covers dynamic knowledge expansion. Activated only when 3+ retries on a single task occur.
- Architecture/structure lessons → `./.hermes/local-patterns.md` (PROJECT-LOCAL)
- Pure technology lessons (API/framework) → `~/.hermes/references/dynamic-patterns.md` (GLOBAL)
- DEFAULT when in doubt → LOCAL (prevents context pollution)

---

## Phase 9 — Workspace Hygiene

For long-running sessions (>1h) or after every Tier 4 completion:
1. Verify `.partial` files are cleaned up or moved to `.hermes/partials/`
2. Confirm pattern cache is not bloated (>20 entries → trigger Phase 4e guardrail 5)
3. Refresh `SessionManager` checkpoint (every 8 turns / 10 min)
4. Drop orphan files: `find . -name "*.tmp" -o -name ".DS_Store" -o -name "__pycache__" | head`
